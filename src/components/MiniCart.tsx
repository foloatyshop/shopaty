...
const MiniCart: FC<MiniCartProps> = ({ toggleSidenav }) => {
  ...
  const cartList =  localStorage.getItem("productCart") ? JSON.parse(localStorage.getItem("productCart")) : [] || state.cart
    ...
    <Box width="100%" maxWidth={380}>
      ...
      {cartList.length > 0 && (
        <Box p={2.5}>
          ...
          >
            Để lại thông tin đặt hàng ({currency(getTotalPrice())})
          </Button>
